...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		pixelExtra := 512 / threads
		pixelStart := 0
		pixelEnd := pixelExtra

		slice := make([]chan [][]uint8, 4)

		for n := 0; n < threads; n++ {
			slice[n] = make(chan [][]uint8)

			go worker(pixelStart, pixelEnd, 0, 512, immutableData, slice[n])
			pixelStart = pixelStart + pixelExtra
			pixelEnd = pixelEnd + pixelExtra
		}

		for n := 0; n < threads; n++ {
			newPixelData = append(newPixelData, <-slice[n]...)
		}
